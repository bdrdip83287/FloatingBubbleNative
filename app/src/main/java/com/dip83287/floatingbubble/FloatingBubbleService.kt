...
// ✅ আপডেট করা ফিক্সড কোড - স্ক্রল করার সময় সিলেকশন মুছে যাবে না
setOnTouchListener(object : View.OnTouchListener {
    ...
    private var isScrollGesture = false // ✅ নতুন ফ্ল্যাগ স্ক্রল ট্র্যাক করার জন্য
    ...
    override fun onTouch(v: View, event: MotionEvent): Boolean {
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                // ✅ বর্তমান Selection সংরক্ষণ করে রাখা
                ...
                // ⚠️ সিলেকশন থাকা অবস্থায় স্পর্শ করলে ACTION_DOWN ড্রেন না করে হ্যান্ডেল করা জরুরি
                return false
            }
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                // ✅ স্ক্রল ডিটেক্ট (Vertical scroll priority)
                if (dy > dx && dy > touchSlop) {
                    isScrollGesture = true
                    v.parent.requestDisallowInterceptTouchEvent(false)
                }

                // ✅ Long press করে টেনে আনলে (Text Drag selection)
                if (isSelecting && (dx > touchSlop || dy > touchSlop)) {
                    isDragging = true
                    handleDragSelection(this@apply, event)
                }

                // ✅ স্ক্রলিং চলাকালীন মুছে যাওয়া সিলেকশন তাৎক্ষণিক ফিরিয়ে আনা
                if (savedSelectionStart >= 0 && savedSelectionEnd >= 0 && !selectionRestored) {
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                ...
                // ✅ ১. স্ক্রলিং শেষ হলে আগের সিলেকশন পুরোপুরি রিস্টোর ও অ্যাকশন বার দেখানো
                if (hasMoved || isScrollGesture) {
                    if (savedSelectionStart >= 0 && savedSelectionEnd >= 0) {
                        ...
                        if (text.isNotEmpty()) {
                            ...
                            showSelectionHandles()
                            updateHandlePositionsImmediate()
                            EmergencyLog.log("Selection maintained after scroll: $savedSelectionStart - $savedSelectionEnd")
                        }
                    }
                } 
                // ✅ ২. কোনো মুভমেন্ট ছাড়াই যদি কেবল একটি Single Tap হয়, তবেই Unselect হবে
                else if (isSingleTap && !hasMoved && !isSelecting && !isDragging && !isScrollGesture) {
                    if (this@apply.hasSelection()) {
