...
setOnTouchListener(object : View.OnTouchListener {
    ...
    override fun onTouch(v: View, event: MotionEvent): Boolean {
        ...
        val resizeHandleView = TextView(this).apply {
            ...
            // Keep the resize icon flush with the bottom zero-line.
            // Positive translationY moves it downward without changing the
            // actual touch/layout bounds of the resize handle.
    ...
    /**
     * Replaces editor text for Undo/Redo without allowing Android EditText
     * to auto-scroll the viewport to the cursor/end of the new text.
     *
     * The exact visible position and selection are captured BEFORE setText(),
     * then restored after layout on the next frames. This is important because
     * setText() can internally reset scroll and setSelection() can request
     * cursor visibility, which otherwise sends the ScrollView to the bottom.
     */
    private fun replaceEditorTextPreservingViewport(
        ...
        fun restoreViewport() {
            try {
                ...
                // setSelection() may itself try to reveal the cursor, so the
                // scroll positions MUST be restored after it.
                editText.scrollTo(editTextScrollX, editTextScrollY)
                scrollView.scrollTo(scrollX, scrollY)
            } catch (_: Exception) {
            }
        }

        // Restore immediately and again after layout/animation frames.
        // No smoothScroll/scrollToDescendant is used, so there is no visible
        // jump toward the cursor.
        restoreViewport()
        editText.post {
            restoreViewport()
            scrollView.post {
    ...
    private fun undoEditorChange() {
        ...
        // Capture exactly what the user was looking at BEFORE Undo.
    ...
    private fun redoEditorChange() {
        ...
        // Capture exactly what the user was looking at BEFORE Redo as well.
        val current = editText.text.toString()
