...
class FloatingBubbleService : Service() {
    ...
    private val DIAGNOSTIC_REPORT_FILE = "restore_save_test_report.txt"
...
private val DELETE_ZONE_HOVER_SCALE = 1.35f
    ...
    override fun onCreate() {
        ...
        try {
            ...
            manualTitleNoteIds.addAll(
                prefs.getStringSet(KEY_MANUAL_TITLE_NOTE_IDS, emptySet())!!
                    .mapNotNull { it.toLongOrNull() }
            )
            loadSavedPositions()

            val freshInstallBeforeMarker = !hasDiagnosticInstallMarker()
            ...
            appendRestoreDiagnosticReport(
                "SERVICE_START: freshInstall=$freshInstallBeforeMarker, " +
                    "notesKeyPresent=$notesKeyWasPresent, " +
                    "JSONchars=${rawNotesJson?.length ?: 0}"
            )

            loadNotes()

            appendRestoreDiagnosticReport(
                "SERVICE_AFTER_LOAD: freshInstall=$freshInstallBeforeMarker, " +
                    "notesKeyPresent=${prefs.contains(STORAGE_NOTES_LIST)}, " +
            ...
            try {
                Toast.makeText(
                    this,
                    "Save/Restore test report updated:\nDownload/Floating Notes/$DIAGNOSTIC_REPORT_FILE",
                    Toast.LENGTH_LONG
                ).show()
            } catch (_: Exception) {
            }
            ...
            appendRestoreDiagnosticReport(
                "SERVICE_STARTUP_ERROR: ${e.javaClass.simpleName}: ${e.message}"
            )
            try {
                Toast.makeText(
                    this,
                    "Restore diagnostic error: ${e.javaClass.simpleName}\nSee Download/Floating Notes/$DIAGNOSTIC_REPORT_FILE",
                    Toast.LENGTH_LONG
                ).show()
            } catch (_: Exception) {
            }
    ...
    /**
     * Restore/save diagnostic for the uninstall -> reinstall test.
     *
     * The report is written to public Downloads/Floating Notes so it survives
     * app uninstall. It records the SharedPreferences state before and after
     * loadNotes(), every saveNotesToPrefs() commit result, and a read-back check.
     * It does NOT store the note contents themselves.
     */
    private fun appendRestoreDiagnosticReport(event: String) {
        try {
            val now = SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS", Locale.US).format(Date())
            val freshInstall = try {
                !File(getNoBackupFilesDir(), DIAGNOSTIC_INSTALL_MARKER).exists()
            } catch (_: Exception) {
                false
            }

            val rawJson = try { prefs.getString(STORAGE_NOTES_LIST, null) } catch (_: Exception) { null }
            val jsonLength = rawJson?.length ?: 0
            val jsonHash = rawJson?.let { sha256(it) } ?: "NONE"
            ...
            sb.append("Fresh-install-marker-present: ${!freshInstall}\n")
            sb.append("SharedPreferences key present: $keyPresent\n")
            sb.append("notes_list JSON chars: $jsonLength\n")
            sb.append("notes_list SHA-256: $jsonHash\n")
            sb.append("Notes count in memory: ${notesList.size}\n")
            sb.append("Notes:\n")
            notesList.forEachIndexed { index, note ->
                sb.append("  ${index + 1}. id=${note.id}, title=${note.title.replace("\n", " ")}, ")
                sb.append("contentChars=${note.content.length}, locked=${note.isLocked}\n")
            }
            sb.append("============================================================\n")

            val existing = readDiagnosticReport()
            writeDiagnosticReport(existing + sb.toString())
        } catch (_: Exception) {
            // Diagnostic must never affect note behavior.
        }
    }
        ...
        return try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                val selectionArgs = arrayOf(
                    ...
                    "${Environment.DIRECTORY_DOWNLOADS}/$DIAGNOSTIC_REPORT_FOLDER/"
                )
                ...
                val file = File(dir, DIAGNOSTIC_REPORT_FILE)
    ...
    private fun writeDiagnosticReport(text: String) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            val collection = android.provider.MediaStore.Downloads.EXTERNAL_CONTENT_URI
            val projection = arrayOf(android.provider.MediaStore.Downloads._ID)
            val selection = "${android.provider.MediaStore.Downloads.DISPLAY_NAME}=? AND ${android.provider.MediaStore.Downloads.RELATIVE_PATH}=?"
            val relativePath = "${Environment.DIRECTORY_DOWNLOADS}/$DIAGNOSTIC_REPORT_FOLDER/"
            val selectionArgs = arrayOf(DIAGNOSTIC_REPORT_FILE, relativePath)
            var uri: Uri? = null
            ...
            if (uri == null) {
                val values = ContentValues().apply {
                    put(android.provider.MediaStore.Downloads.DISPLAY_NAME, DIAGNOSTIC_REPORT_FILE)
            ...
            File(dir, DIAGNOSTIC_REPORT_FILE).writeText(text, Charsets.UTF_8)
        }
    }
    ...
    private fun loadNotes() {
        appendRestoreDiagnosticReport("LOAD_NOTES_ENTERED")

        val notesJson = try {
            prefs.getString(STORAGE_NOTES_LIST, "")
        } catch (e: Exception) {
            appendRestoreDiagnosticReport("LOAD_NOTES_PREFS_READ_ERROR: ${e.javaClass.simpleName}: ${e.message}")
            throw e
        }
        ...
        if (!notesJson.isNullOrEmpty()) {
            try {
                ...
                appendRestoreDiagnosticReport("LOAD_NOTES_JSON_PARSED_SUCCESSFULLY")
            } catch (e: Exception) {
                appendRestoreDiagnosticReport("LOAD_NOTES_GSON_ERROR: ${e.javaClass.simpleName}: ${e.message}")
                if (notesList.isEmpty()) {
                    notesList.add(NoteItem(System.currentTimeMillis(), "Untitled Note", ""))
                    appendRestoreDiagnosticReport("LOAD_NOTES_FALLBACK_DEFAULT_CREATED")
                }
            }
        } else {
            if (notesList.isEmpty()) {
                notesList.add(NoteItem(System.currentTimeMillis(), "Untitled Note", ""))
                appendRestoreDiagnosticReport("LOAD_NOTES_EMPTY_PREFS_DEFAULT_CREATED")
            } else {
                appendRestoreDiagnosticReport("LOAD_NOTES_EMPTY_PREFS_MEMORY_ALREADY_NONEMPTY")
            }
        }

        saveNotesToPrefs()
        appendRestoreDiagnosticReport("LOAD_NOTES_COMPLETED")
    }

    /**
     * Saves the complete note database synchronously and verifies the write by
     * reading notes_list back immediately from SharedPreferences.
     */
    private fun saveNotesToPrefs() {
        val notesJson = Gson().toJson(notesList)
        val expectedHash = sha256(notesJson)
        ...
        val readBack = try { prefs.getString(STORAGE_NOTES_LIST, null) } catch (_: Exception) { null }
        val readBackHash = readBack?.let { sha256(it) } ?: "NONE"
        val verified = committed && readBack == notesJson

        appendRestoreDiagnosticReport(
            "SAVE_NOTES: commitReturned=$committed, " +
