...
class FloatingBubbleService : Service() {
    ...
    private val scrollUpdateHandler = Handler(Looper.getMainLooper())
    ...
    // --- Custom selection handles/logic are REMOVED.
    // --- Native Android EditText selection handles will be used.

    // Example of createFullNotePad where native selection is enabled:
    private fun createFullNotePad(): LinearLayout {
        ...
        editText = EditText(this).apply {
            ...
            //----- This line enables native selection handles -----
            setTextIsSelectable(true)
            // Optional: If needed, add a context action mode callback for custom menu
            /*
        ...
        // RecyclerView for your notes list, action buttons etc.
        ...
        // Other buttons or controls as you need (optional)

        return container
    }

    // Placeholder for your NoteAdapter - keep as is
    class NoteAdapter(private val notes: List<NoteItem>) :
        ...
        class NoteViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
            ...
            val contentView: TextView = itemView.findViewById(android.R.id.text2)
        }
    }

    // -- If you have floatingActionBar etc. for copy/share, keep your old logic for them --

    private fun hideFloatingActionBar() {
        if (floatingActionBar != null && isActionBarVisible && actionBarWindowManager != null) {
            try {
    ...
    // ... Other functions, as needed (unchanged, unless they used legacy handles) ...
...
class FloatingBubbleService : Service() {
    ...
    // ✅ CRITICAL FIX: Update handle positions based on selection boundaries (follows selected text)
    private fun updateHandlePositions() {
        ...
        // Right handle - position at end of selection (moves with text)
    ...
    // ✅ Trigger handle position update during scrolling
    private fun scheduleHandleUpdateDuringScroll() {
        if (!editText.hasSelection()) return
        
        scrollUpdateRunnable?.let { scrollUpdateHandler.removeCallbacks(it) }
        ...
        scrollUpdateHandler.postDelayed(runnable, 16) // ~60fps smooth update
    }
    ...
    private fun openEditorForNote(note: NoteItem) {
        ...
        scrollView = ScrollView(this).apply {
            ...
            // ✅ Listen to scroll changes to update handle positions in real-time
            viewTreeObserver.addOnScrollChangedListener {
                if (editText.hasSelection()) {
                    // Update handle positions during scroll
                    updateHandlePositions()
