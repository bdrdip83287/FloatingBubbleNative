...
class FloatingBubbleService : Service() {
    ...
    private val diagnosticStageReports = mutableListOf<String>()
...
private val DELETE_ZONE_HOVER_SCALE = 1.35f
    ...
    private fun createThreeStageDiagnosticMarker() {
        try {
            ...
            if (!marker.exists()) marker.writeText("createdAt=" + SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS", Locale.US).format(Date()) + "\npackage=" + packageName + "\n")
        } catch (e: Exception) { try { EmergencyLog.logException(e, "three-stage diagnostic marker") } catch (_: Exception) {} }
    }

    private fun diagnosticSha256(value: String): String = try {
        val digest = java.security.MessageDigest.getInstance("SHA-256")
        digest.digest(value.toByteArray(Charsets.UTF_8)).joinToString("") { "%02x".format(it) }
    } catch (_: Exception) { "ERROR" }

    private fun diagnosticVersionName(): String = try { packageManager.getPackageInfo(packageName, 0).versionName ?: "unknown" } catch (_: Exception) { "unknown" }
    ...
    private fun captureThreeStageDiagnostic(stage: String) {
        try {
            val sb = StringBuilder()
            val prefsFile = File(dataDir, "shared_prefs/$PREFS_NAME.xml")
            sb.append("============================================================\n")
            sb.append("FLOATING NOTES THREE-STAGE RESTORE SOURCE DIAGNOSTIC\n")
            ...
            sb.append("APK versionName: ").append(diagnosticVersionName()).append("\n")
            sb.append("APK versionCode: ").append(diagnosticVersionCode()).append("\n")
            sb.append("Fresh-install-at-startup: ").append(diagnosticFreshInstall).append("\n")
            sb.append("dataDir: ").append(dataDir.absolutePath).append("\n")
            sb.append("noBackupFilesDir: ").append(noBackupFilesDir.absolutePath).append("\n")
            sb.append("FLAG_ALLOW_BACKUP: ").append((applicationInfo.flags and android.content.pm.ApplicationInfo.FLAG_ALLOW_BACKUP) != 0).append("\n")
            sb.append("prefsXmlPath: ").append(prefsFile.absolutePath).append("\n")
            sb.append("prefsXmlExists: ").append(prefsFile.exists()).append("\n")
            var xml = ""
            if (prefsFile.exists()) {
                try { xml = prefsFile.readText() } catch (_: Exception) {}
                sb.append("prefsXmlLength: ").append(xml.length).append("\n")
                sb.append("prefsXmlLastModified: ").append(prefsFile.lastModified()).append("\n")
                sb.append("prefsXmlSHA256: ").append(diagnosticSha256(xml)).append("\n")
                sb.append("prefsXmlContainsTargetTitle: ").append(xml.contains(DIAG_TARGET_TITLE)).append("\n")
                sb.append("prefsXmlContainsTargetId: ").append(xml.contains(DIAG_TARGET_ID.toString())).append("\n")
                sb.append("prefsXmlContainsTargetLastEdited: ").append(xml.contains(DIAG_TARGET_LAST_EDITED.toString())).append("\n")
                sb.append("prefsXmlContainsNotesList: ").append(xml.contains("notes_list")).append("\n")
            }
            if (::prefs.isInitialized) {
                val json = prefs.getString(STORAGE_NOTES_LIST, "") ?: ""
                sb.append("prefsKeyPresent: ").append(prefs.contains(STORAGE_NOTES_LIST)).append("\n")
                sb.append("notesJsonChars: ").append(json.length).append("\n")
                sb.append("notesJsonSHA256: ").append(diagnosticSha256(json)).append("\n")
                try {
                    val type = object : com.google.gson.reflect.TypeToken<List<NoteItem>>() {}.type
                    val raw: List<NoteItem> = com.google.gson.Gson().fromJson(json, type) ?: emptyList()
                    sb.append("rawParsedNoteCount: ").append(raw.size).append("\n")
                    raw.forEachIndexed { i, note -> sb.append("  rawNote").append(i+1).append(": id=").append(note.id).append(", title=").append(note.title).append(", contentChars=").append(note.content.length).append(", lastEdited=").append(note.lastEdited).append(", targetMatch=").append(note.title == DIAG_TARGET_TITLE || note.id == DIAG_TARGET_ID).append("\n") }
                } catch (e: Exception) { sb.append("rawParseError: ").append(e.javaClass.simpleName).append(": ").append(e.message).append("\n") }
            } else sb.append("prefsKeyPresent: NOT_INITIALIZED\n")
            sb.append("inMemoryNoteCount: ").append(notesList.size).append("\n")
            notesList.forEachIndexed { i, note -> sb.append("  memoryNote").append(i+1).append(": id=").append(note.id).append(", title=").append(note.title).append(", contentChars=").append(note.content.length).append(", lastEdited=").append(note.lastEdited).append(", targetMatch=").append(note.title == DIAG_TARGET_TITLE || note.id == DIAG_TARGET_ID).append("\n") }
            sb.append("APP_DATA_SCAN_TARGET: ").append(DIAG_TARGET_TITLE).append("\n")
            var hits = 0
            fun scan(dir: File) {
                if (hits >= 50 || !dir.exists()) return
                try { dir.listFiles()?.forEach { f ->
                    if (hits >= 50) return@forEach
                    if (f.isDirectory) { if (!f.absolutePath.contains("/cache/") && !f.absolutePath.contains("/code_cache/")) scan(f) }
                    else if (f.length() <= 5_000_000L && f.extension.lowercase() in listOf("xml","json","txt","log")) try {
                        val text=f.readText()
                        if (text.contains(DIAG_TARGET_TITLE) || text.contains(DIAG_TARGET_ID.toString())) { hits++; sb.append("  HIT: ").append(f.absolutePath).append(" (length=").append(text.length).append(", lastModified=").append(f.lastModified()).append(")\n") }
                    } catch (_: Exception) {}
                } } catch (_: Exception) {}
            }
            scan(dataDir)
            sb.append("appDataTargetHitCount: ").append(hits).append("\n")
            val oldJson = File(getExternalFilesDir(null), "floating_notes_backup.json")
            sb.append("oldAppSpecificJsonExists: ").append(oldJson.exists()).append("\n")
            sb.append("TARGET_TITLE: ").append(DIAG_TARGET_TITLE).append("\n")
            sb.append("TARGET_ID: ").append(DIAG_TARGET_ID).append("\n")
            sb.append("TARGET_LAST_EDITED_REFERENCE: ").append(DIAG_TARGET_LAST_EDITED).append("\n")
            sb.append("============================================================\n")
            diagnosticStageReports.add(sb.toString())
        } catch (e: Exception) { diagnosticStageReports.add("STAGE=$stage\nDIAGNOSTIC_ERROR=${e.javaClass.simpleName}: ${e.message}\n") }
    }
    ...
    private fun writeDiagnosticReportToDownloads(content: String) {
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                ...
                var oldUri: android.net.Uri? = null
                resolver.query(collection, arrayOf(MediaStore.Downloads._ID), "${MediaStore.Downloads.DISPLAY_NAME}=? AND ${MediaStore.Downloads.RELATIVE_PATH}=?", arrayOf(DIAG_REPORT_FILE, "${Environment.DIRECTORY_DOWNLOADS}/$DIAG_REPORT_FOLDER/"), null)?.use { c -> if (c.moveToFirst()) oldUri = ContentUris.withAppendedId(collection, c.getLong(0)) }
                if (oldUri != null) resolver.delete(oldUri!!, null, null)
                val values = ContentValues().apply { put(MediaStore.Downloads.DISPLAY_NAME, DIAG_REPORT_FILE); put(MediaStore.Downloads.MIME_TYPE, "text/plain"); put(MediaStore.Downloads.RELATIVE_PATH, "${Environment.DIRECTORY_DOWNLOADS}/$DIAG_REPORT_FOLDER/"); put(MediaStore.Downloads.IS_PENDING, 1) }
                val uri = resolver.insert(collection, values) ?: return
                resolver.openOutputStream(uri, "w")?.use { it.write(content.toByteArray(Charsets.UTF_8)) }
                val done = ContentValues().apply { put(MediaStore.Downloads.IS_PENDING, 0) }
                resolver.update(uri, done, null, null)
            } else {
                val folder = File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), DIAG_REPORT_FOLDER)
                if (!folder.exists()) folder.mkdirs()
                File(folder, DIAG_REPORT_FILE).writeText(content, Charsets.UTF_8)
            }
        } catch (e: Exception) { try { EmergencyLog.logException(e, "diagnostic report MediaStore") } catch (_: Exception) {} }
    }
